...
namespace KatlaSport.Services.ProductManagement
{
    ...
    public class ProductCatalogueService : IProductCatalogueService
    {
        ...
        {
            var dbCategories = await _context.Categories.Where(p => p.Id == productCategoryId).ToArrayAsync().ConfigureAwait(false);
        ...
        {
            var dbProducts = await _context.Products.Where(p => p.Code == createRequest.Code).ToArrayAsync().ConfigureAwait(false);
            if (dbProducts.Length > 0)
            {
        ...
        {
            var dbProducts = await _context.Products.Where(p => p.Code == updateRequest.Code && p.Id != productId).ToArrayAsync().ConfigureAwait(false);
            if (dbProducts.Length > 0)
            {
                throw new RequestedResourceHasConflictException("code");
            }

            dbProducts = await _context.Products.Where(p => p.Id == productId).ToArrayAsync().ConfigureAwait(false);
            if (dbProducts.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
        ...
        {
            var products = await _context.Products.Where(p => p.Id == productId).ToArrayAsync().ConfigureAwait(false);
        ...
        {
            var products = await _context.Products.Where(p => productId == p.Id).ToArrayAsync().ConfigureAwait(false);
